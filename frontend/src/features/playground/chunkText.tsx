      ...
      {segs.map((s, i) =>
        s.kind === 'text' ? (
          text.slice(s.from, s.to).trim() ? (
            <p key={i} className="whitespace-pre-wrap break-words text-body text-text-secondary">{highlight(text, spans, s.from, s.to)}</p>
          ) : null
        ) : (
...
export function ProseChunk({ text, spans, from = 0, className }: { text: string; spans: Span[]; from?: number; className?: string }) {
  return (
    <p className={cn('whitespace-pre-wrap break-words text-body text-text-secondary', className)}>
      {from > 0 && <span className="text-text-tertiary">… </span>}
      {highlight(text, spans, from)}
    </p>
  )
}
